namespace DelegateDecompiler
{
	...
	[SuppressMessage("ReSharper", "ArrangeBraces_ifelse")]
	...
	internal class Processor
	{
		class ProcessorState
		{
			...
			public ProcessorState(
				...
				IDictionary<FieldInfo, Address> delegates = null)
			{
				Delegates = delegates ??
				            new Dictionary<FieldInfo, Address>();
				Stack = stack;
				Locals = locals;
				Args = args;
				Instruction = instruction;
			...
			public ProcessorState Clone
				(Instruction instruction, Instruction last = null)
			{
				var state = new ProcessorState
					            (null, new VariableInfo[Locals.Length], Args.ToArray(), instruction, last, Delegates);
				var addressMap = new Dictionary<Address, Address>();
				var buffer = Stack.Select(address => address.Clone(addressMap)).ToList();
			...
			public void Merge(Expression test, ProcessorState leftState, ProcessorState rightState)
			{
				...
				for ( var i = 0; i < leftState?.Locals.Length; i++ )
				{
					...
					Locals[i].Address =
						Address.Merge(test, leftLocal?.Address, rightLocal?.Address, addressMap);

				}

				var buffer = new List<Address>();
				while ( leftState?.Stack.Count > 0 ||
				        rightState?.Stack.Count > 0 )
				{
			...
			public Expression Final() => Stack.Count == 0
				                             ? Expression.Empty()
				                             : Stack.Pop();
		}
		...
		static readonly MethodInfo StringConcat = typeof( string ).GetMethod
			...
			Type returnType)
		{
			...
			processor._states.Push(new ProcessorState(new Stack<Address>(), locals, args, instruction));

			var ex = AdjustType(processor.Process(), returnType);

			if ( ex.Type != returnType &&
		...
		readonly Stack<ProcessorState> _states = new Stack<ProcessorState>();
		...
		Expression Process()
		{
			...
			while ( _states.Count > 0 )
			{
				...
				if ( state?.Instruction != null &&
				     state?.Instruction != state?.Last )
				{
					Debug.WriteLine(state.Instruction);

					if ( state.Instruction.OpCode != OpCodes.Nop &&
					     state.Instruction.OpCode != OpCodes.Break )
					{
						if ( state.Instruction.OpCode == OpCodes.Ldtoken )
						{
							...
							LdArg(state, 3);
						else if ( state.Instruction.OpCode == OpCodes.Ldarg_S ||
						          state.Instruction.OpCode == OpCodes.Ldarg ||
						          state.Instruction.OpCode == OpCodes.Ldarga ||
						          state.Instruction.OpCode == OpCodes.Ldarga_S )
						{
						...
						else if ( state.Instruction.OpCode == OpCodes.Ldelema ||
						          state.Instruction.OpCode == OpCodes.Ldelem_Any ||
						...
						else if ( state.Instruction.OpCode == OpCodes.Stelem_Any ||
						...
						else if ( state.Instruction.OpCode == OpCodes.Ldfld ||
						          state.Instruction.OpCode == OpCodes.Ldflda )
						{
						...
						else if ( state.Instruction.OpCode == OpCodes.Ldsfld )
						{
							...
							if ( IsCachedAnonymousMethodDelegate(field) )
							{
								Address address;
								state.Stack.Push
								      (state.Delegates.TryGetValue(field, out address) ? address : Expression.Field(null, field));
							}
							else
						...
						else if ( state.Instruction.OpCode == OpCodes.Stfld )
						{
							...
							if ( newExpression == null )
							{
								...
								if ( memberInitExpression != null )
								{
									...
									var bindings = new List<MemberBinding>(memberInitExpression.Bindings)
									               {
										               Expression.Bind(field, value)
									               };
							...
							else instance.Expression = Expression.MemberInit(newExpression, Expression.Bind(field, value));
						}
						...
						else if ( state.Instruction.OpCode == OpCodes.Br_S ||
						          state.Instruction.OpCode == OpCodes.Br )
						{
						...
						{
							state.Instruction = ConditionalBranch
								(state, val => Expression.Equal(val, Default(val.Type)));
							continue;
						}
						          ...
						          state.Instruction.OpCode == OpCodes.Brtrue_S )
						{
							...
							{
								state.Instruction = ConditionalBranch
									(state, val => Expression.NotEqual(val, Default(val.Type)));
								continue;
							}
						}
						          ...
						          state.Instruction.OpCode == OpCodes.Beq_S )
						{
							var val1 = state.Stack.Pop();
							state.Instruction = ConditionalBranch
								(state, val => AdjustedBinaryExpression(val, val1, ExpressionType.Equal));
							continue;
						}
						          ...
						          state.Instruction.OpCode == OpCodes.Bne_Un_S )
						{
							var val1 = state.Stack.Pop();
							state.Instruction = ConditionalBranch
								(state, val => AdjustedBinaryExpression(val, val1, ExpressionType.NotEqual));
							continue;
						}
						...
						else if ( state.Instruction.OpCode == OpCodes.Add )
						{
							...
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.Add));
						}
						else if ( state.Instruction.OpCode == OpCodes.Add_Ovf ||
						          state.Instruction.OpCode == OpCodes.Add_Ovf_Un )
						{
							var val1 = state.Stack.Pop();
							var val2 = state.Stack.Pop();
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.AddChecked));
						}
						else if ( state.Instruction.OpCode == OpCodes.Sub )
						{
							...
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.Subtract));
						}
						else if ( state.Instruction.OpCode == OpCodes.Sub_Ovf ||
						          state.Instruction.OpCode == OpCodes.Sub_Ovf_Un )
						{
							var val1 = state.Stack.Pop();
							var val2 = state.Stack.Pop();
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.SubtractChecked));
						}
						else if ( state.Instruction.OpCode == OpCodes.Mul )
						{
							...
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.Multiply));
						}
						else if ( state.Instruction.OpCode == OpCodes.Mul_Ovf ||
						          state.Instruction.OpCode == OpCodes.Mul_Ovf_Un )
						{
							var val1 = state.Stack.Pop();
							var val2 = state.Stack.Pop();
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.MultiplyChecked));
						}
						else if ( state.Instruction.OpCode == OpCodes.Div ||
						          state.Instruction.OpCode == OpCodes.Div_Un )
						{
							var val1 = state.Stack.Pop();
							var val2 = state.Stack.Pop();
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.Divide));
						}
						else if ( state.Instruction.OpCode == OpCodes.Rem ||
						          state.Instruction.OpCode == OpCodes.Rem_Un )
						{
							var val1 = state.Stack.Pop();
							var val2 = state.Stack.Pop();
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.Modulo));
						}
						else if ( state.Instruction.OpCode == OpCodes.Xor )
						{
							...
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.ExclusiveOr));
						}
						else if ( state.Instruction.OpCode == OpCodes.Shl )
						{
							...
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.LeftShift));
						}
						else if ( state.Instruction.OpCode == OpCodes.Shr ||
						          state.Instruction.OpCode == OpCodes.Shr_Un )
						{
							var val1 = state.Stack.Pop();
							var val2 = state.Stack.Pop();
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.RightShift));
						}
						...
						else if ( state.Instruction.OpCode == OpCodes.Conv_Ovf_I ||
						          state.Instruction.OpCode == OpCodes.Conv_Ovf_I_Un )
						{
							var val1 = state.Stack.Pop();
							state.Stack.Push(Expression.ConvertChecked(val1, typeof( int ))); // Suppot x64?
						}
						else if ( state.Instruction.OpCode == OpCodes.Conv_Ovf_I1 ||
						          state.Instruction.OpCode == OpCodes.Conv_Ovf_I1_Un )
						{
							var val1 = state.Stack.Pop();
							state.Stack.Push(Expression.ConvertChecked(val1, typeof( sbyte )));
						}
						else if ( state.Instruction.OpCode == OpCodes.Conv_Ovf_I2 ||
						          state.Instruction.OpCode == OpCodes.Conv_Ovf_I2_Un )
						{
							var val1 = state.Stack.Pop();
							state.Stack.Push(Expression.ConvertChecked(val1, typeof( short )));
						}
						else if ( state.Instruction.OpCode == OpCodes.Conv_Ovf_I4 ||
						          state.Instruction.OpCode == OpCodes.Conv_Ovf_I4_Un )
						{
							var val1 = state.Stack.Pop();
							state.Stack.Push(Expression.ConvertChecked(val1, typeof( int )));
						}
						else if ( state.Instruction.OpCode == OpCodes.Conv_Ovf_I8 ||
						          state.Instruction.OpCode == OpCodes.Conv_Ovf_I8_Un )
						{
							var val1 = state.Stack.Pop();
							state.Stack.Push(Expression.ConvertChecked(val1, typeof( long )));
						}
						else if ( state.Instruction.OpCode == OpCodes.Conv_Ovf_U ||
						          state.Instruction.OpCode == OpCodes.Conv_Ovf_U_Un )
						{
							var val1 = state.Stack.Pop();
							state.Stack.Push(Expression.ConvertChecked(val1, typeof( uint ))); // Suppot x64?
						}
						else if ( state.Instruction.OpCode == OpCodes.Conv_Ovf_U1 ||
						          state.Instruction.OpCode == OpCodes.Conv_Ovf_U1_Un )
						{
							var val1 = state.Stack.Pop();
							state.Stack.Push(Expression.ConvertChecked(val1, typeof( byte )));
						}
						else if ( state.Instruction.OpCode == OpCodes.Conv_Ovf_U2 ||
						          state.Instruction.OpCode == OpCodes.Conv_Ovf_U2_Un )
						{
							var val1 = state.Stack.Pop();
							state.Stack.Push(Expression.ConvertChecked(val1, typeof( ushort )));
						}
						else if ( state.Instruction.OpCode == OpCodes.Conv_Ovf_U4 ||
						          state.Instruction.OpCode == OpCodes.Conv_Ovf_U4_Un )
						{
							var val1 = state.Stack.Pop();
							state.Stack.Push(Expression.ConvertChecked(val1, typeof( uint )));
						}
						else if ( state.Instruction.OpCode == OpCodes.Conv_Ovf_U8 ||
						          state.Instruction.OpCode == OpCodes.Conv_Ovf_U8_Un )
						{
							var val1 = state.Stack.Pop();
							state.Stack.Push(Expression.ConvertChecked(val1, typeof( ulong )));
						}
						else if ( state.Instruction.OpCode == OpCodes.Conv_R4 ||
						          state.Instruction.OpCode == OpCodes.Conv_R_Un )
						{
							var val1 = state.Stack.Pop();
						...
						else if ( state.Instruction.OpCode == OpCodes.Call ||
						          state.Instruction.OpCode == OpCodes.Callvirt )
						{
						...
						else if ( state.Instruction.OpCode == OpCodes.Ceq )
						{
							...
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.Equal));
						}
						else if ( state.Instruction.OpCode == OpCodes.Cgt ||
						          state.Instruction.OpCode == OpCodes.Cgt_Un )
						{
							var val1 = state.Stack.Pop();
							var val2 = state.Stack.Pop();
							...
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.GreaterThan));
						}
						else if ( state.Instruction.OpCode == OpCodes.Clt ||
						          state.Instruction.OpCode == OpCodes.Clt_Un )
						{
							var val1 = state.Stack.Pop();
							var val2 = state.Stack.Pop();
							...
							state.Stack.Push
							      (AdjustedBinaryExpression
								       (val2, AdjustBooleanConstant(val1, val2.Type), ExpressionType.LessThan));
						}
						else if ( state.Instruction.OpCode == OpCodes.Isinst )
						{
							var val = state.Stack.Pop();
							if ( state.Instruction.Next != null &&
							     state.Instruction.Next.OpCode == OpCodes.Ldnull &&
							     state.Instruction.Next.Next != null &&
							     state.Instruction.Next.Next.OpCode == OpCodes.Cgt_Un )
							{
		...
		static object GetRuntimeHandle(object operand)
		{
			...
			if ( fieldInfo != null )
				return fieldInfo.DeclaringType.TypeHandle;

			var methodBase = operand as MethodBase;
			if ( methodBase != null )
				return methodBase.DeclaringType.TypeHandle;
		...
		static bool IsCachedAnonymousMethodDelegate(FieldInfo field)
		{
			...
			return field.Name.StartsWith(CachedAnonymousMethodDelegate) &&
			       Attribute.IsDefined(field, typeof( CompilerGeneratedAttribute ), false) ||
			       field.Name.StartsWith(CachedAnonymousMethodDelegateRoslyn) && field.DeclaringType != null &&
			       Attribute.IsDefined(field.DeclaringType, typeof( CompilerGeneratedAttribute ), false);
		}
		...
		static Expression Box(Expression expression, Type type)
		{
			...
			{
				return expression.Type.GetTypeInfo().IsEnum
		...
		static Expression ConvertEnumExpressionToUnderlyingType(Expression expression) =>
			expression.Type.GetTypeInfo().IsEnum
				? Expression.Convert(expression, expression.Type)
				: expression;

		static Expression Default(Type type) =>
			Expression.Constant(type.GetTypeInfo().IsValueType ? Activator.CreateInstance(type) : null, type);
		...
		Instruction ConditionalBranch(ProcessorState state, Func<Expression, BinaryExpression> condition)
		{
			...
			_states.Push(rightState);
			_states.Push(leftState);
		...
		class JoinPointState
		{
			...
			public Stack<Instruction> CurrentInstructions => Left != null ? LeftInstructions : RightInstructions;
		}

		static Instruction GetJoinPoint(Instruction left, Instruction right)
		{
			var joinPointStates = new Stack<JoinPointState>();
			...
			while ( joinPointStates.Count > 0 )
			{
				...
				if ( joinPointState.Right == null )
				{
					...
					{
						if ( joinPointState.RightInstructions.Count <= 0 ||
						     leftInstruction != joinPointState.RightInstructions.Pop() )
							break;
				...
				else
				{
					...
					else
					{
						...
						switch ( joinPointState.Current.OpCode.FlowControl )
						{
							...
							case FlowControl.Cond_Branch:
								joinPointState.Pending = new JoinPointState
									                         ((Instruction) joinPointState.Current.Operand, joinPointState.Current.Next);
		...
		internal static Expression AdjustType(Expression expression, Type type)
		{
			...
			if ( !type.IsAssignableFrom(expression.Type) &&
			     expression.Type.GetTypeInfo().IsEnum &&
			     expression.Type.GetTypeInfo().GetEnumUnderlyingType() == type )
				return Expression.Convert(expression, type);

			return type.GetTypeInfo().IsValueType != expression.Type.GetTypeInfo().IsValueType 
				? Expression.Convert(expression, type) 
				: expression;
		}
		...
		{
			if ( type != typeof( bool ) ||
			     expression.Type != typeof( int ) ) return expression;

			var constantExpression = expression as ConstantExpression;
			return constantExpression != null
				       ? Expression.Constant(!Equals(constantExpression.Value, 0))
				       : expression;
		}
		...
		static void LdC(ProcessorState state, int i) => state?.Stack.Push(Expression.Constant(i));

		static void LdC(ProcessorState state, long i) => state?.Stack.Push(Expression.Constant(i));

		static void LdC(ProcessorState state, float i) => state?.Stack.Push(Expression.Constant(i)); 
		...
		static Expression BuildMethodCallExpression(MethodInfo m, Address instance, Expression [] arguments)
		{
			if ( m.Name == "Add" &&
			     ...
			     typeof( IEnumerable ).IsAssignableFrom(instance.Type) )
			{
				var newExpression = instance.Expression as NewExpression;
				if ( newExpression != null )
				{
			...
			if ( m.Name != "InitializeArray" ||
				...
				return Expression.Call(instance.Expression != null ? instance : null, m, arguments);
			{
		...
		static IList<Expression> GetExpressionsForStringConcat(IList<Expression> arguments)
		{
		...
		static void StLoc(ProcessorState state, int index)
		{
			...
			if ( info != null ) info.Address = Processor.AdjustType(state.Stack.Pop(), info.Type);
		}
