namespace DelegateDecompiler
{
	...
	internal class OptimizeExpressionVisitor : ExpressionVisitor
	{
		...
		protected override Expression VisitNew(NewExpression node)
			=>
			IsNullable(node.Type) && node.Arguments.Count == 1
				? Expression.Convert(this.Visit(node.Arguments[0]), node.Type)
				: base.VisitNew(node);

		static bool IsNullable(Type type)
			=>
			type.GenericTypeArguments.Length > 0 && type.GetGenericTypeDefinition() == typeof( Nullable<> );
		...
		protected override Expression VisitConditional(ConditionalExpression node)
		{
			...
			if ( binaryExpression != null )
			{
				BinaryExpression result;
				if ( ifTrueBinary != null &&
				     TryConvert2(binaryExpression, ifTrueBinary, out result) )
					return result;
		...
		private static bool TryConvertOrElse(
			Expression hasValue,
			BinaryExpression getValueOrDefault,
			out BinaryExpression result)
		{
			var hasValueBinary = hasValue as BinaryExpression;
			if ( hasValueBinary != null &&
			     TryConvert2(hasValueBinary, getValueOrDefault, out result) )
			{
				return true;
			}
			if ( hasValue.NodeType == ExpressionType.Not &&
			     TryConvert1(( (UnaryExpression) hasValue ).Operand, getValueOrDefault, out result) )
			{
				return true;
			}
		...
		private static bool TryConvertAndAlso(
			Expression hasValue,
			BinaryExpression getValueOrDefault,
			out BinaryExpression result)
		{
		...
		private static bool TryConvert2(
			BinaryExpression hasValue,
			BinaryExpression getValueOrDefault,
			out BinaryExpression result)
		{
			Expression left;
		...
		private static bool TryConvert1(Expression hasValue, BinaryExpression getValueOrDefault, out BinaryExpression result)
		{
			...
			if ( ExtractNullableArgument(hasValue, getValueOrDefault.Left, out expression) )
			{
				result = Expression.MakeBinary
					(getValueOrDefault.NodeType, expression, Expression.Convert(getValueOrDefault.Right, expression.Type));
				return true;
			}
			...
			{
				result = Expression.MakeBinary
					(getValueOrDefault.NodeType, Expression.Convert(getValueOrDefault.Left, expression.Type), expression);
				return true;
			}
		...
		private static bool ExtractNullableArgument(
			Expression hasValue,
			...
			out Expression expression)
		{
			...
			if ( IsHasValue(hasValue, out memberExpression) &&
			     IsGetValueOrDefault(getValueOrDefault, out callExpression) )
			{
		...
		private static bool IsHasValue(Expression expression, out MemberExpression property)
		{
			...
			return property != null && property.Member.Name == "HasValue" && property.Expression != null &&
			       IsNullable(property.Expression.Type);
		}
		...
		private static bool IsGetValueOrDefault(Expression expression, out MethodCallExpression method)
		{
			...
			return method != null && method.Method.Name == "GetValueOrDefault" && method.Object != null &&
			       IsNullable(method.Object.Type);
		}
